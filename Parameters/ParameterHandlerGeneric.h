...
/// @brief Class responsible for handling of systematic error parameters with
/// different types defined in the config. Like spline, normalisation parameters
/// etc.
...
public:
  /// @brief Constructor
  /// @param FileNames A vector of strings representing the YAML files used for
  ...
  SplineInterpolation GetParSplineInterpolation(const int i) const {
    return SplineParams.at(i).SplineInterpolationType;
  }
  /// @brief Get the interpolation types for splines affecting a particular
  /// SampleName
  /// @ingroup ParameterHandlerGetters
  const std::vector<SplineInterpolation>
  GetSplineInterpolationFromSampleName(const std::string &SampleName);
  ...
  std::string GetParSplineName(const int i) const {
    return SplineParams.at(i).spline_name;
  }

  /// @brief DB Get spline parameters depending on given SampleName
  /// @ingroup ParameterHandlerGetters
  const std::vector<int>
  GetGlobalSystIndexFromSampleName(const std::string &SampleName,
                                   const SystType Type);
  /// @brief EM: value at which we cap spline knot weight
  /// @param i spline parameter index, not confuse with global index
  /// @ingroup ParameterHandlerGetters
  double GetParSplineKnotUpperBound(const int i) const {
    return SplineParams.at(i).SplineKnotUpBound;
  }
  /// @brief EM: value at which we cap spline knot weight
  /// @param i spline parameter index, not confuse with global index
  /// @ingroup ParameterHandlerGetters
  double GetParSplineKnotLowerBound(const int i) const {
    return SplineParams.at(i).SplineKnotLowBound;
  }

  /// @brief DB Grab the number of parameters for the relevant SampleName
  /// @param SampleName property of SampleHandler class based on which we select
  /// whether to apply uncertainties or not
  /// @param Type Type of syst, for example kNorm, kSpline etc
  /// @ingroup ParameterHandlerGetters
  int GetNumParamsFromSampleName(const std::string &SampleName,
                                 const SystType Type);
  ...
  GetParsNamesFromSampleName(const std::string &SampleName,
                             const SystType Type);
  ...
  /// @ingroup ParameterHandlerGetters
  const std::vector<int>
  GetParsIndexFromSampleName(const std::string &SampleName,
                             const SystType Type);

  /// @brief DB Get spline parameters depending on given SampleName
  /// @ingroup ParameterHandlerGetters
  const std::vector<std::string>
  GetSplineParsNamesFromSampleName(const std::string &SampleName);
  /// @brief DB Get spline parameters depending on given SampleName
  /// @ingroup ParameterHandlerGetters
  const std::vector<std::string>
  GetSplineFileParsNamesFromSampleName(const std::string &SampleName);

  /// @brief DB Grab the Spline Modes for the relevant SampleName
  /// @ingroup ParameterHandlerGetters
  const std::vector<std::vector<int>>
  GetSplineModeVecFromSampleName(const std::string &SampleName);

  /// @brief DB Get norm/func parameters depending on given SampleName
  /// @ingroup ParameterHandlerGetters
  const std::vector<NormParameter>
  GetNormParsFromSampleName(const std::string &SampleName) const;
  /// @brief HH Get functional parameters for the relevant SampleName
  /// @ingroup ParameterHandlerGetters
  const std::vector<FunctionalParameter>
  GetFunctionalParametersFromSampleName(const std::string &SampleName) const;
  /// @brief KS: Grab the Spline parameters for the relevant SampleName
  /// @ingroup ParameterHandlerGetters
  const std::vector<SplineParameter>
  GetSplineParsFromSampleName(const std::string &SampleName) const;
  ...
  /// @brief KS Function to set to prior parameters of a given group or values
  /// from vector
  /// @param Group name of group, like Xsec or Flux
  ...
  /// @brief Dump Matrix to ROOT file, useful when we need to pass matrix info
  /// to another fitting group
...
protected:
  ...
  ///
  /// This template function provides a way to iterate over parameters
  /// associated with a specific Sample ID (SampleName). It applies a filter
  /// function to determine which parameters to process and an action function
  /// to define what to do with the selected parameters.
  ///
  /// @tparam FilterFunc The type of the filter function used to determine
  /// which parameters to include.
  /// @tparam ActionFunc The type of the action function applied to each
  /// selected parameter.
  /// @param SampleName The Sample ID used to filter parameters.
  template <typename FilterFunc, typename ActionFunc>
  void IterateOverParams(const std::string &SampleName,
                         FilterFunc const &filter, ActionFunc const &action);

  /// @brief Initializes the systematic parameters from the configuration file.
  /// This function loads parameters like normalizations and splines from the
  /// provided YAML file.
  ...
  /// The YAML file defines the types of systematic errors, interpolation types,
  /// and bounds for splines.
  ...
  NormParameter GetNormParameter(const YAML::Node &param);

  /// @brief Get Osc params
  /// @param param Yaml node describing param
  /// @param Index Global parameter index
  OscillationParameter GetOscillationParameters(const YAML::Node &param);

  /// @brief Get Func params
  /// @param param Yaml node describing param
  /// @param Index Global parameter index
  FunctionalParameter GetFunctionalParameters(const YAML::Node &param);
  /// @brief Get Spline params
  /// @param param Yaml node describing param
  /// @param Index Global parameter index
  SplineParameter GetSplineParameter(const YAML::Node &param);

  /// @brief Retrieve parameters that apply to a given sample name.
  /// @tparam ParamT Type of parameter (e.g., FunctionalParameter,
  /// NormParameter).
  /// @param indexMap Map from local to global parameter indices.
  /// @param params Vector of all parameters of the given type.
  /// @param SampleName The name of the sample to filter applicable parameters
  /// for.
  /// @return Vector of parameters of type ParamT that apply to the specified
  /// sample.
  template <typename ParamT>
  std::vector<ParamT>
  GetTypeParamsFromSampleName(const std::map<int, int> &indexMap,
                              const std::vector<ParamT> &params,
                              const std::string &SampleName) const;

  std::vector<TypeParameterBase *> GlobalParams;

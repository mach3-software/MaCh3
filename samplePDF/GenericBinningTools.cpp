...
std::unique_ptr<TH1> GetGenericBinningTH1(samplePDFFDBase &sample,
  ...
  int naxes = static_cast<int>(binning.Axes.size());
  if (naxes == 1) {
...
GetGenericBinningTH1Slice(samplePDFFDBase &sample,
  ...
  if (slice_definition.size() != static_cast<std::size_t>(binning.GetNDimensions())) {
    MACH3LOG_ERROR(
...
std::vector<std::unique_ptr<TH1>> GetGenericBinningTH1Slices(
  ...
  if (static_cast<int>(slice_ax) >= binning.GetNDimensions()) {
...
std::vector<std::unique_ptr<TH2>> GetGenericBinningTH2Slices(
  ...
  if ((slice_axes[0] == slice_axes[1]) ||
    (slice_axes[0] >= static_cast<std::size_t>(binning.GetNDimensions())) ||
    (slice_axes[1] >= static_cast<std::size_t>(binning.GetNDimensions()))) {

...
class LIBCONFIGXX_API Setting
{
  ...
  Setting & operator=(bool value) throw();
  Setting & operator=(int value) throw();
  Setting & operator=(long value) throw();
  Setting & operator=(const long long &value) throw();
  Setting & operator=(const double &value) throw();
  Setting & operator=(float value) throw();
  Setting & operator=(const char *value) throw();
  Setting & operator=(const std::string &value) throw();
  ...
  const Setting & getParent() const throw();
  Setting & getParent() throw();
...
class LIBCONFIGXX_API Config
{
  ...
  void read(FILE *stream) throw();
  void write(FILE *stream) const;

  void readString(const char *str) throw();
  inline void readString(const std::string &str) throw()
  { return(readString(str.c_str())); }

  void readFile(const char *filename) throw();
  void writeFile(const char *filename) throw();

  inline Setting & lookup(const std::string &path) const
    throw()
  { return(lookup(path.c_str())); }

  Setting & lookup(const char *path) const throw();

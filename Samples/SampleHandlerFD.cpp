  ...
  for(unsigned int iEvent = 0; iEvent < GetNEvents(); ++iEvent){
    ...
    if (ParHandler) {
      ...
      for (std::vector<NormParameter>::iterator it = norm_parameters.begin(); it != norm_parameters.end(); ++it) {
        ...
        bool ModeMatch = MatchCondition(it->modes, MCEvents[iEvent].mode);
        if (!ModeMatch) {
      ...
      for(int iChannel = 0; iChannel < GetNOscChannels(iSample); iChannel++) {
        ...
        for (unsigned int iEvent = 0; iEvent < GetNEvents(); iEvent++) {
          ...
          if (!MCEvents[iEvent].isNC && Channel == iChannel) {
            EnergyArray.push_back(MCEvents[iEvent].enu_true);
          }
        }
        ...
        if (MCEvents[0].rw_truecz != M3::_BAD_DOUBLE_) {
          for (unsigned int iEvent = 0; iEvent < GetNEvents(); iEvent++) {
            ...
            //DB Remove NC events from the arrays which are handed to the NuOscillator objects
            if (!MCEvents[iEvent].isNC && Channel == iChannel) {
              CosineZArray.push_back(MCEvents[iEvent].rw_truecz);
            }
          }
...
// ************************************************
  ...
  if (MCEvents[iEvent].isNC) {
    ...
    if (MCEvents[iEvent].rw_truecz != M3::_BAD_DOUBLE_) {
      //Atmospherics
      osc_w_pointer = Oscillator->GetNuOscillatorPointers(Sample, OscIndex, InitFlav, FinalFlav, MCEvents[iEvent].enu_true, MCEvents[iEvent].rw_truecz);
    } else {
      //Beam
      osc_w_pointer = Oscillator->GetNuOscillatorPointers(Sample, OscIndex, InitFlav, FinalFlav, MCEvents[iEvent].enu_true);
    }
  } // end if NC
...
// ************************************************
  ...
  const int Mode = MCEvents[Event].mode;

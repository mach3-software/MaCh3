...
MaCh3 has been developed by a wide range of people, many of them PhD students, and we maintain this spirit of inclusive development.

The gitflow development policy should be followed, with the main and develop branches only being committed to via pull request.

New features should be developed on branches in this repository with the branch naming: `username/feature/FEATURENAME`
...
- **`breaking:`** For changes that break backward compatibility or require changes for experiment part of MaCh3
...
MaCh3 uses an **autogenerated Doxygen configuration file** (`Doc/doxyfile`).

To generate the documentation locally:
```bash
./Doc/setup_wiki.sh       # Download wiki used by doxygen
mkdir build
cd build
cmake ..                  # Inject the current MaCh3 version
cd ..
doxygen Doc/doxyfile      # Generate the documentation
```
The Doxygen configuration automatically includes the current software version from CMake (taken from PROJECT_VERSION), so we don’t have to update the version manually when a new tag is created.

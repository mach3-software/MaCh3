...
namespace MaCh3Plotting{    
    ...
    InputManager::InputManager(std::string translationTomlName){
        ...
        // go through each of the parameters and sample arrays and keep track of which ones we've been told about
        for(std::string parameter: toml_h::find<std::vector<std::string>>(parameters_toml, "Parameters"))
        {
    ...
    void InputManager::addFile(std::string fileName){
        ...
        // EM: need to be done in this order since fillFileData needs to know info about the file, e.g. fitter and what things are in it 
    ...
    /// If printLevel is "summary", will loop through all the files known to this InputManager and call InputFile::Summarise().
    /// If printLevel is "dump", will print all parameters known to this InputManager and then loop through all input files and call InputFile::Dump().
    ...
    const float InputManager::GetPostFitError(int fileNum, std::string paramName, std::string errorType) const{
        const InputFile *inputFileDef = fileVec[fileNum];
        
        // set default type if not specified
        if (errorType == "") errorType = inputFileDef->defaultErrorType;

        if(inputFileDef->postFitErrors.find(errorType) == inputFileDef->postFitErrors.end()){
            std::cerr << "ERROR: " << __FILE__ << ":" << __LINE__ << std::endl;
            std::cerr << "  requested error type, " << "\"" << errorType << "\" does not exist in the specified file: " << std::endl;
            std::cerr << "    " << inputFileDef->fileName << std::endl;
            std::cerr << "    at index " << fileNum << std::endl;
            throw;
        } 

        if(inputFileDef->postFitErrors.at(errorType).find(paramName) != inputFileDef->postFitErrors.at(errorType).end()){
            return inputFileDef->postFitErrors.at(errorType).at(paramName);
        }
        return __BAD_FLOAT__;
    }

    const float InputManager::GetPostFitValue(int fileNum, std::string paramName, std::string errorType) const{
        const InputFile *inputFileDef = fileVec[fileNum];
        ...
        if(inputFileDef->postFitValues.at(errorType).find(paramName) != inputFileDef->postFitValues.at(errorType).end()){
    ...
    std::vector<std::string> InputManager::parseLocation(std::string locationString, fitterEnum fitter, fileTypeEnum fileType, std::string parameter, std::string sample) const {
        std::vector<std::string> tokens;
        
        std::size_t pos = 0;
        std::string toReplace = "";

        // loop through the raw location string and replace any instance of {PARAMETER} with the name of the parameter
        toReplace = "{PARAMETER}";
        while ( (pos = locationString.find(toReplace)) != std::string::npos )
        {
            locationString.replace(pos, toReplace.length(), getFitterSpecificParamName(fitter, fileType, parameter));
        }
        ...
        while ( (pos = locationString.find(toReplace)) != std::string::npos )
        {
            locationString.replace(pos, toReplace.length(), getFitterSpecificSampleName(fitter, fileType, sample));
        }
    ...
    TObject *InputManager::findRootObject(InputFile *fileDef, std::vector<std::string> locationVec) const {
        ...
        // if vector has two elements, interpret the first as the directory and the second as a string to match with the end of the objects in the file
        else if ( locationVec.size() == 2 )
        {
            ...
            {
                // loop through the keys in the directory and find objects whose name matches the specified pattern
            ...
            if ( nMatchingObjects > 1 )
            {
                ...
                if ( locationVec.size() == 2 ) std::cerr << locationVec[1];
                std::cerr << std::endl;
                std::cerr << "Found " << nMatchingObjects << " matching objects, should just be one" << std::endl;
                throw;
            }
        ...
        else
        {
            ...
            std::cerr << "Should have two elements: [ (directory to look in), (end of the name of the object) ]" << std::endl;
    ...
    // helper function to read from the translation toml to get an option for a particular sub toml (e.g. Parameters or Samples)
    // returns true and sets "ret" if the option is specified for this fitter and parameter
    // otherwise returns false
    template <typename T>
    bool InputManager::getFitterSpecificOption(fitterEnum fitter, std::string option, T *ret, std::string parameter, toml::value subToml) const{
        if (toml_h::contains(subToml, parameter)){
            ...
            if(toml_h::contains(paramTranslation, convertFitterNames(fitter))){
                ...
                toml::value fitterParamTranslation = toml_h::find(paramTranslation, convertFitterNames(fitter));
    ...
    bool InputManager::findBySampleLLH(InputFile *inputFileDef, std::string parameter, fitterEnum fitter, std::string sample, bool setInputFileScan){

        toml::value thisFitterSpec_toml = toml_h::find(fitterSpec_toml, convertFitterNames(fitterEnum(fitter)));
        ...
        std::vector<std::string> testLLHRawLocations = toml_h::find<std::vector<std::string>>(testLLHToml, "location");
        ...
        {
            LLHObj = findRootObject(inputFileDef, parseLocation(rawLocation, fitter, kLLH, parameter, sample));
            if (LLHObj != nullptr)
            {
                break;
            }
        ...
        if(LLHObj == nullptr) return false;
        ...
        if(setInputFileScan){
            ...
            if(LLHObjType == "TH1D"){
                ...
                std::cerr << "ERROR: uknown type of LLH object specified: " << LLHObjType << ", can tell me how to read it here" << std::endl;
                std::cerr << "       " << __FILE__ << ":" << __LINE__ << std::endl;
                throw; 
            }
    ...
    bool InputManager::findPostFitParamError(InputFile *inputFileDef, std::string parameter, fitterEnum fitter, std::string errorType, bool setInputFileError){
        ...
        std::vector<std::string> postFitLocations = toml_h::find<std::vector<std::string>>(specificErrorType, "Loc");
        ...
        if(getFitterSpecificParamOption<std::vector<std::string>>(fitter, "postFitLoc", &postFitLocations_override, parameter)){
        ...
        for(std::string postFitLoc: postFitLocations){
            ...
            if(postFitErrors == NULL) continue;
    ...
    // EM: Lots of room for improvement in fillFileInfo and fillFileData, should be split up into more methods, currently a lot of copy pasting
    void InputManager::fillFileInfo(InputFile *inputFileDef, bool printThoughts){
        ...
        if(printThoughts) std::cout << "Checking contents of file " << inputFileDef->fileName << std::endl; 
        ...
        for (int i =1; i < kNFitters - 1; i++){
            ...
            if(printThoughts) std::cout << "Checking if this is a " << convertFitterNames(fitter) << " file" << std::endl;
            ...
            {
                std::cerr << "ERROR " << __FILE__ << ":" << __LINE__ << std::endl;
                std::cerr << "translation toml doesnt contain a definition for fitter " << convertFitterNames(fitter) << std::endl;
                throw;
            }
            ...
            for( std::string LLHType: {"sample", "penalty", "total"} )
            {
                ...
                if(printThoughts) std::cout << ".... searching for " << LLHType << " LLH scans... ";
                ...
                for(std::string parameter: knownParameters){
                    ...
                    for ( std::string rawLocation: testLLHRawLocations)
                    {
                        if ( findRootObject(inputFileDef, parseLocation(rawLocation, fitter, kLLH, parameter)) != nullptr )
                        {
                            numLLHParams ++;
                ...
                if(printThoughts) std::cout << "Found " << numLLHParams << std::endl;
                ...
                /// \todo add a check here to make sure all the scans that are in the file are being picked up by the reader, and warn if any are not being used
            }

            if(printThoughts) std::cout << std::endl;
            ...
            if(printThoughts) {
                ...
                if(toml_h::contains<std::string>(thisFitterSpec_toml, "defaultPostFitErrorType"))
                {
                    ...
                    toml::value defaultErrorType = toml_h::find(postFitErrorSpec, toml_h::find<std::string>(thisFitterSpec_toml, "defaultPostFitErrorType"));
                    std::vector<std::string> locations = toml_h::find<std::vector<std::string>>(defaultErrorType, "Loc");
                    for (std::string loc: locations) std::cout << std::endl << loc;
                    std::cout << ")" << std::endl;
                }
                else
                {
            ...
            std::string defaultErrorType = toml_h::find<std::string>(thisFitterSpec_toml, "defaultPostFitErrorType");
            for(std::string parameter: knownParameters)
            {
            ...
            if(printThoughts) std::cout << "........Found " << numPostFitParams << std::endl;
            ...
            // This is really just a check to see if the number of by sample LLH scans is the same as normal LLH scans
            if(printThoughts)
            ...
            for(std::string sample: knownSamples)
            {
                ...
                if(numLLHBySampleParams != 0){
                    ...
                    if(printThoughts)
                            std::cout << "........Found " << numLLHBySampleParams << " LLH scans for sample " << sample << std::endl;

                    if((numLLHParams != numLLHBySampleParams)){
                        std::cerr << "ERROR: hmmmmm something weird is happening here" << std::endl;
                        std::cerr << "       I have " << numLLHBySampleParams << " LLH scans for sample " << sample << " But " << numLLHParams << " total LLH parameters " << std::endl;
                        std::cerr << "       If you are at peace with this and understand why this is you can come here and remove the throw, otherwise you should investigate whats going on here" << std::endl; 
                        std::cerr << __FILE__ << ":" << __LINE__ << std::endl;
                        throw;
                    }
                }
            ...
            */
            ...
            if(foundFitter){
                if(printThoughts) std::cout << "This is a " << convertFitterNames(fitterEnum(i)) << " File!" << std::endl;
        ...
        if(printThoughts) std::cout << "I don't know what kinda fitter this came from, will proceed with caution" << std::endl;
    ...
    void InputManager::fillFileData(InputFile *inputFileDef, bool printThoughts){
        ...
        if(printThoughts)std::cout << "....getting data from file " << inputFileDef->fileName << std::endl; 

        toml::value thisFitterSpec_toml = toml_h::find(fitterSpec_toml, convertFitterNames(inputFileDef->fitter));

        // set the default post fit error type so we can read it as default later
        inputFileDef->defaultErrorType = toml_h::find<std::string>(thisFitterSpec_toml, "defaultPostFitErrorType");

        // ########### First fill up the LLH vectors ############
        for(std::string LLHType: {"sample", "penalty", "total"})
        {
            if( !inputFileDef->hasLLHScans_map.at(LLHType) ) continue;
            ...
            for(std::string parameter: inputFileDef->availableParams_LLH)
            {
                ...
                for ( std::string rawLocation: testLLHRawLocations )
                {
                    LLHObj = findRootObject(inputFileDef, parseLocation(rawLocation, inputFileDef->fitter, kLLH, parameter));
                    // if we found it then break out of the loop
                    if( LLHObj != nullptr ) break;
                }

                // double check that we actually found it.. just in case
                if( LLHObj == nullptr )
                {
                    std::cerr << "Hmmm, something seems to have gone wrong and I couldnt find the " << LLHType << " LLH scan for " << parameter << " when attempting to read the data for it" << std::endl;
                    std::cerr << __FILE__ << ":" << __LINE__ << std::endl;
                }
                ...
                // EM: maybe have the type of the LLH object specified in the toml to know what to downcast it to??
                if(LLHObjType == "TH1D")
                {
                    LLHGraph = new TGraph((TH1D*) LLHObj);
                    delete LLHObj;
        ...
        {
            std::vector <std::string> availableErrorTypes = toml_h::find<std::vector<std::string>>(thisFitterSpec_toml, "AvailablePostFitErrorTypes");
    ...
    std::string InputManager::MaCh3ToBANFFName(std::string mach3ParamName){
        ...
        if (mach3ParamName.rfind("b_", 0) == 0) BANFFName = BANFFfluxName(BANFFName);
        
        if(!mach3ParamName.find("ndd_")) return NOT_FOUND_STR;
    ...
    std::string InputManager::BANFFfluxName(std::string mach3ParamName){
        // EM: Keep hard coding of BANFF flux parameter names because I cant think of a way of simplifying the name conversion :(    
        ...
        if (i != std::string::npos) BANFFName.erase(i, Delete.length());
        ...
        if(FluxNumber<11) BANFFprefix = ("NDNuModeNumu"+std::to_string(FluxNumber) );
        
        else if(FluxNumber-11<5) BANFFprefix= "NDNuModeNumub" +std::to_string(FluxNumber-11);

        else if(FluxNumber-11-5<7) BANFFprefix= "NDNuModeNue" + std::to_string(FluxNumber-11-5);

        else if(FluxNumber-11-5-7 < 2) BANFFprefix= "NDNuModeNueb" + std::to_string(FluxNumber-11-5-7);

        else if(FluxNumber-11-5-7-2 < 5) BANFFprefix= "NDANuModeNumu" + std::to_string(FluxNumber-11-5-7-2);

        else if(FluxNumber-11-5-7-2-5< 11) BANFFprefix= "NDANuModeNumub" + std::to_string(FluxNumber-11-5-7-2-5);

        else if(FluxNumber-11-5-7-2-5-11< 2) BANFFprefix= "NDANuModeNue" + std::to_string(FluxNumber-11-5-7-2-5-11);

        else if(FluxNumber-11-5-7-2-5-11-2< 7) BANFFprefix= "NDANuModeNueb" + std::to_string(FluxNumber-11-5-7-2-5-11-2);

        // Now replace the "ND" part with "SK" to make the SK names
        if(isSKflux)BANFFprefix.replace(BANFFprefix.find("ND"), 2, std::string("SK"));
    ...
    // EM: annoyingly, need a different function for gundam names for postfit errors than LLH scans as file structure is totally different, and so we also need to find the gundam file directory that the parameter is in
    std::string InputManager::getGundamParamName_PostFit(std::string mach3ParamName, TDirectoryFile *fitDir, TH1D *PostFitError_ret, Int_t *parIdx){
        ...
        if((mach3ParamName.find("b_") != std::string::npos) && mach3ParamName.size() < 5){ //we got a flux parameter
            ...
            for(gundamBinIdx = 0; gundamBinIdx <= PostFitErrors->GetNbinsX(); gundamBinIdx++){
                ...
                // EM: we check that mach3ParamName_reduced is at the very end of binLabel, so that we correctly find double digit numbers,
                // e.g. if we a re looking for OOFV_1, it will be found in OOFV_11, OOFV_12 etc.
                if(binLabel== gundamName)break;
            }
        }

        else if((mach3ParamName.find("ND Det ") != std::string::npos)){ //we got a ND Cov parameter
            TDirectoryFile *systDir = (TDirectoryFile*)errorDir->Get("ND280 Detector Systematics (variations parameters)");
            TDirectoryFile *valuesDir = (TDirectoryFile*)systDir->Get("values");
            PostFitErrors = (TH1D*)valuesDir->Get("postFitErrors_TH1D");

            std::string mach3ParamName_reduced = std::string(mach3ParamName);
            mach3ParamName_reduced.erase(mach3ParamName_reduced.find("ND Det "), std::string("ND Det ").length());
            gundamName = "#" + mach3ParamName_reduced;
            ...
            bool found = false;
            for (std::string paramType: {"Cross-Section (binned) Systematics", "Cross-Section Systematics", "ND280 Detector Systematics (splined detector parameters)", "ND280 Detector Systematics (variations parameters)"}){
            TDirectoryFile *systDir = (TDirectoryFile*)errorDir->Get(paramType.c_str());
            TDirectoryFile *valuesDir = (TDirectoryFile*)systDir->Get("values");
            PostFitErrors = (TH1D*)valuesDir->Get("postFitErrors_TH1D");

            // EM: strip out the NDS_ at the beginning of the near detector systematic parameteters, this isnt there in gundam
            std::string mach3ParamName_reduced = std::string(mach3ParamName);
            if(mach3ParamName_reduced.find("NDS_") != std::string::npos)
                mach3ParamName_reduced.erase(mach3ParamName_reduced.find("NDS_"), std::string("NDS_").length());

            else if(mach3ParamName.find("EB_dial_") != std::string::npos){
                mach3ParamName_reduced.erase(mach3ParamName_reduced.find("EB_dial_"), std::string("EB_dial_").length());
            ...
            for(gundamBinIdx = 0; gundamBinIdx <= PostFitErrors->GetNbinsX(); gundamBinIdx++){
                ...
                binLabel.find(mach3ParamName_reduced) == binLabel.size() - mach3ParamName_reduced.size()){
                gundamName = binLabel;
        ...
        // EM: fill the optional return values, the error TH1D that this parameter was found in and the index where it was found
        if(PostFitError_ret !=NULL) *PostFitError_ret = *PostFitErrors;
        if(parIdx != NULL) *parIdx = gundamBinIdx;

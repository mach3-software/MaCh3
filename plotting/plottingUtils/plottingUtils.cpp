...
namespace MaCh3Plotting{
    ...
    /// It will go through your provided graph and make a histogram binning by taking the midpoints of all the graph points x values, then fill the histogram with the graphs y values. 
    /// For the first and last points it will extend the binning out of the graph bounds using the width between the outermost and second outermost points. 
    /// This can be useful if e.g. you want to draw cumulative stacks of LLH scans.
    TH1D TGraphToTH1D(TGraph graph, std::string newName, std::string newTitle){
        std::string name;
        std::string title;

        if(newName == "") name = graph.GetName();
        else name = newName;

        if(newTitle == "") title = graph.GetTitle();
        else title = newTitle;

...
namespace MaCh3Plotting{
    ...
    /// - Instantiate a StyleManager using the style config toml specified in the plotting config toml, or if none was provided, using DEFAULT_STYLE_TOML.
    /// - Instantiate an InputManager using the translation config toml specified in the plotting config toml, or if none was provided, using DEFAULT_TRANSLATION_TOML.
    /// - Add all files specified in this PlottingManagers FileNames vector to the new InputManager that was just created
    /// \warning This should always be called *After* ParseInputs() unless you are manually specifying all input file names and config toml names in your drawing application.
    void PlottingManager::Initialise(){
        ...
        if ( translationToml == "" ) translationToml = DEFAULT_TRANSLATION_TOML;

        std::string styleToml = toml_h::find<std::string>(managerOptions, "styleToml");
        if ( styleToml == "" ) styleToml = DEFAULT_STYLE_TOML;
    ...
    ///   -l <FileLabelStr> string of labels for each file in the format Label1;Label2;... to be parsed by parseFileLabels(), number of labels should match exactly the number of files
    ///   -c <PlottingConfig> specify a plotting config toml file to be used in place of the default one when initialising this manager
    ///   -d <DrawOptions> extra draw options to be passed when making plots, see https://root.cern/doc/master/classTHistPainter.html#HP01a for examples
    ///  
    ...
    /// \endcode 
    /// \todo make this able to return any un-parsed arguments so that user can specify their own arguments for use in their plotting scripts
    void PlottingManager::ParseInputs(int argc, char **argv){
        ...
        if((FileLabels.size() != 0) &&(FileLabels.size() != FileNames.size())){
            std::cerr<<"ERROR: hmmm, you gave me "<< FileLabels.size() << " labels but "<< FileNames.size()<<" files"<<std::endl;
    ...
    /// \todo Would be good to add functionality to this to allow user to add their own options.
    /// \todo The way I can think to do this would be have fn addUserOption() to set the options, defining the cmd line option (e.g. -x), the name of the option, and maybe some description of the option to be used in the help message
    /// \todo can then store these options in some map or something to be retrieved later by getUserOption()
    ...
    void PlottingManager::usage(){
        /// \todo Inplement this. 
        /// \todo could add some function to allow user to specify the help message for their particular script, then auto generate what the cmd line syntax looks like based on user specified options?
    }

    /// Will check the provided saveName for file extenstions, if it is one of .pdf or .eps, then just use the provided string as the full output name.
    /// If no file extension is specified, append .pdf so plots will be saved as pdf.
    /// if some other file extension is specified, replace with .pdf as only .pdf and .eps support printing multiple plots to one file in root.
    void PlottingManager::setOutFileName(std::string saveName){
        ...
        while(ext.find(".") != std::string::npos) ext.erase(0, ext.find(".") + 1);
        if(ext == "pdf" || ext == "ps" || ext == "eps"){
            OutputName = saveName;
            return;            
        } 

        std::cout <<"WARNING: file extension " << ext << " that you provided doesnt support multiple plots in one file" << std::endl;
        std::cout << "        should be one of .pdf, .eps .ps, will use pdf" <<std::endl;
        OutputName = saveName +".pdf";
    }

    /// Output file name, including the file extension will be returned, but with specified suffix after the name but before the extension.
    /// This is useful for e.g. saving multiple LLH scan types to separate files: can specify suffix "_PriotLLH" will return OutputName_PriorLLH.ext
    /// \todo Make this support .root files too
    ...
    void PlottingManager::SetExec(std::string execName){
        ...
        if(OutputName == "Plot.pdf") setOutFileName(GetOption<std::string>("defaultOutputName"));
    }
    
    void PlottingManager::parseFileLabels(std::string labelString, std::vector<std::string> &labelVec){
        // take in a string defining labels of the form "label1;label2;...;labelN" and parse it into a vector containing the labels

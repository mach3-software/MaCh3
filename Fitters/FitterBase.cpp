...
#pragma GCC diagnostic ignored "-Wuseless-cast"
    ...
    FitterBase::FitterBase(manager *const man)
    : fitMan(man) {
  // *************************
  AlgorithmName = "";
  // Get mach3 modes from manager
  random = std::make_unique<TRandom3>(
      Get<int>(fitMan->raw()["General"]["Seed"], __FILE__, __LINE__));
  ...
  auto outfile = Get<std::string>(fitMan->raw()["General"]["OutputFile"],
                                  __FILE__, __LINE__);
  // Save output every auto_save steps
  // you don't want this too often
  // https://root.cern/root/html606/TTree_8cxx_source.html#l01229
  auto_save = Get<int>(fitMan->raw()["General"]["MCMC"]["AutoSave"], __FILE__,
                       __LINE__);
  ...
  // Auto-save every 200MB, the bigger the better
  // https://root.cern/root/html606/TTree_8cxx_source.html#l01229
  ...
  TotalNSamples = 0;
  fTestLikelihood = GetFromManager<bool>(
      fitMan->raw()["General"]["Fitter"]["FitTestLikelihood"], false, __FILE__,
      __LINE__);
}
...
void FitterBase::SaveSettings() {
  ...
  // KS: It is better to use experiment specific header file. If given
  // experiment didn't provide it we gonna use one given by Core MaCh3.
  if (!file) {
  ...
  MACH3LOG_WARN("\033[0;31mCurrent Total RAM usage is {:.2f} GB\033[0m",
                MaCh3Utils::getValue("VmRSS") / 1048576.0);
  MACH3LOG_WARN("\033[0;31mOut of Total available RAM {:.2f} GB\033[0m",
                MaCh3Utils::getValue("MemTotal") / 1048576.0);

  MACH3LOG_INFO("#####Current Setup#####");
  MACH3LOG_INFO("Number of covariances: {}", systematics.size());
  for (unsigned int i = 0; i < systematics.size(); ++i)
    MACH3LOG_INFO("{}: Cov name: {}, it has {} params", i,
                  systematics[i]->GetName(),
                  systematics[i]->GetNumSystematicParams());
  MACH3LOG_INFO("Number of SampleHandlers: {}", samples.size());
  for (unsigned int i = 0; i < samples.size(); ++i) {
    MACH3LOG_INFO("{}: SampleHandler name: {}, it has {} samples", i,
                  samples[i]->GetName(), samples[i]->GetNsamples());
    for (int iSam = 0; iSam < samples[i]->GetNsamples(); ++iSam) {
      MACH3LOG_INFO("   {}: Sample name: {}, with {} osc channels", iSam,
                    samples[i]->GetSampleTitle(iSam),
                    samples[i]->GetNOscChannels(iSam));
    }
  }
  // TN: Have to close the folder in order to write it to disk before SaveOutput
...
void FitterBase::PrepareOutput() {
  ...
  if (OutputPrepared)
    return;
  // MS: Check if we are fitting the test likelihood, rather than T2K
  // likelihood, and only setup T2K output if not
  if (!fTestLikelihood) {
    ...
    // Do we want to save proposal? This will break plotting scripts and is
    // heave for disk space and step time. Only use when debugging
    bool SaveProposal = GetFromManager<bool>(
        fitMan->raw()["General"]["SaveProposal"], false, __FILE__, __LINE__);

    if (SaveProposal)
      MACH3LOG_INFO("Will save in the chain proposal parameters and LogL");
    ...
    if (SaveProposal)
      outTree->Branch("LogLProp", &logLProp, "LogLProp/D");
...
void FitterBase::SaveOutput() {
  ...
  MACH3LOG_INFO("{} steps took {:.2e} seconds to complete. ({:.2e}s / step).",
                step - stepStart, clock->RealTime(),
                clock->RealTime() / static_cast<double>(step - stepStart));
  MACH3LOG_INFO("{} steps were accepted.", accCount);
#ifdef DEBUG
  if (debug) {
    debugFile << "\n\n"
              << step << " steps took " << clock->RealTime()
              << " seconds to complete. (" << clock->RealTime() / step
              << "s / step).\n"
...
void FitterBase::AddSampleHandler(SampleHandlerBase *const sample) {
    ...
    for (int iExisting = 0; iExisting < s->GetNsamples(); ++iExisting) {
      for (int iNew = 0; iNew < sample->GetNsamples(); ++iNew) {
        if (s->GetSampleTitle(iExisting) == sample->GetSampleTitle(iNew)) {
          MACH3LOG_ERROR("Duplicate sample title '{}' in handler {} detected: "
                         "same title exist in handler ",
                         sample->GetSampleTitle(iNew), sample->GetName(),
                         s->GetName());
          throw MaCh3Exception(__FILE__, __LINE__);
        }
      }
  ...
  MACH3LOG_INFO("Adding {} object, with {} samples", sample->GetName(),
                sample->GetNsamples());
...
// *************************
  MACH3LOG_INFO("Adding systematic object {}, with {} params", cov->GetName(), cov->GetNumSystematicParams());
  ...
  {
    for (int iPar = 0; iPar < systematics[s]->GetNumSystematicParams(); ++iPar)
    {
      for (int i = 0; i < cov->GetNumSystematicParams(); ++i)
      {
  ...
  CovFolder->cd();
  std::vector<double> n_vec(cov->GetNumSystematicParams());
  for (int i = 0; i < cov->GetNumSystematicParams(); ++i) {
  ...
  if (!Config.IsNull()) {
    TMacro ConfigSave =
        YAMLtoTMacro(Config, (std::string("Config_") + cov->GetName()));
    ConfigSave.Write();
  }
...
void FitterBase::StartFromPreviousFit(const std::string &FitName) {
  ...
  for (size_t s = 0; s < systematics.size(); ++s) {
    ...
    // KS: Not every covariance uses yaml, if it uses yaml make sure they are
    // identical
    if (ConfigCov != nullptr) {
      ...
      if (!compareYAMLNodes(CovSettings, ConfigCurrent)) {
        MACH3LOG_ERROR("Yaml configs in previous chain (from path {}) and "
    ...
    MACH3LOG_INFO("Printing new starting values for: {}",
                  systematics[s]->GetName());
    systematics[s]->PrintNominalCurrProp();

    // Resetting branch addressed to nullptr as we don't want to write into a
    // delected vector out of scope...
    for (int i = 0; i < systematics[s]->GetNumSystematicParams(); ++i) {
...
void FitterBase::ProcessMCMC() {
  ...
  // Process the MCMC
  if (GetFromManager<bool>(fitMan->raw()["General"]["ProcessMCMC"], false,
                           __FILE__, __LINE__)) {
    // Make the processor
    ...
    if (!outputFile->IsOpen()) {
      ...
      outputFile = new TFile(
          Get<std::string>(fitMan->raw()["General"]["Output"]["Filename"],
                           __FILE__, __LINE__)
              .c_str(),
          "UPDATE");
    }
...
void FitterBase::DragRace(const int NLaps) {
  ...
  MACH3LOG_INFO("All tests will be performed with {} threads",
                M3::GetNThreads());
  ...
  for (unsigned int ivs = 0; ivs < samples.size(); ++ivs) {
    ...
    clockRace.Stop();
    MACH3LOG_INFO("It took {:.4f} s to reweights {} times sample: {}",
                  clockRace.RealTime(), NLaps, samples[ivs]->GetName());
  ...
  for (unsigned int ivs = 0; ivs < samples.size(); ++ivs) {
    ...
    clockRace.Stop();
    MACH3LOG_INFO(
        "It took {:.4f} s to calculate  GetLikelihood {} times sample:  {}",
        clockRace.RealTime(), NLaps, samples[ivs]->GetName());
    MACH3LOG_INFO("On average {:.6f}", clockRace.RealTime() / NLaps);
  }
  // Get vector of proposed steps. If we want to run LLH scan or something else
  // after we need to revert changes after proposing steps multiple times
  ...
  for (size_t s = 0; s < systematics.size(); ++s) {
    ...
    clockRace.Stop();
    MACH3LOG_INFO("It took {:.4f} s to propose step {} times cov:  {}",
                  clockRace.RealTime(), NLaps, systematics[s]->GetName());
    MACH3LOG_INFO("On average {:.6f}", clockRace.RealTime() / NLaps);
  }
  for (size_t s = 0; s < systematics.size(); ++s) {
  ...
  for (size_t s = 0; s < systematics.size(); ++s) {
    ...
    clockRace.Stop();
    MACH3LOG_INFO(
        "It took {:.4f} s to calculate  get likelihood {} times cov:  {}",
        clockRace.RealTime(), NLaps, systematics[s]->GetName());
    MACH3LOG_INFO("On average {:.6f}", clockRace.RealTime() / NLaps);
  }
...
// *************************
bool FitterBase::GetScanRange(
    std::map<std::string, std::vector<double>> &scanRanges) const {
  // *************************
  bool isScanRanges = false;
  // YSP: Set up a mapping to store parameters with user-specified ranges,
  // suggested by D. Barrow
  if (fitMan->raw()["LLHScan"]["ScanRanges"]) {
    ...
    MACH3LOG_INFO("There are no user-defined parameter ranges, so I'll use "
                  "default param bounds for LLH Scans");
  }
  return isScanRanges;
}
...
void FitterBase::RunLLHScan() {
  ...
  // KS: Turn it on if you want LLH scan for each ND sample separately, which
  // increase time significantly but can be useful for validating new samples or
  // dials.
  bool PlotLLHScanBySample = GetFromManager<bool>(
      fitMan->raw()["LLHScan"]["LLHScanBySample"], false, __FILE__, __LINE__);
  auto SkipVector = GetFromManager<std::vector<std::string>>(
      fitMan->raw()["LLHScan"]["LLHScanSkipVector"], {}, __FILE__, __LINE__);

  // Now finally get onto the LLH scan stuff
  // Very similar code to MCMC but never start MCMC; just scan over the
  // parameter space
  ...
  if (PlotLLHScanBySample) {
    ...
    for (unsigned int ivs = 0; ivs < samples.size(); ++ivs) {
      for (int is = 0; is < samples[ivs]->GetNsamples(); ++is) {
        SampleSplit_LLH[SampleIterator] = outputFile->mkdir(
            (samples[ivs]->GetSampleTitle(is) + "_LLH").c_str());
        SampleIterator++;
      }
    }
  }
  // Number of points we do for each LLH scan
  const int n_points = GetFromManager<int>(
      fitMan->raw()["LLHScan"]["LLHScanPoints"], 100, __FILE__, __LINE__);
  double nSigma = GetFromManager<int>(fitMan->raw()["LLHScan"]["LLHScanSigma"],
                                      1., __FILE__, __LINE__);
  ...
  for (ParameterHandlerBase *cov : systematics) {
    ...
    for (int i = 0; i < npars; ++i) {
      ...
      if (isScanRanges) {
        ...
        if (it != scanRanges.end() &&
            it->second.size() ==
                2) { // Making sure the range is has only two entries
          lower = it->second[0];
          upper = it->second[1];
          MACH3LOG_INFO(
              "Found matching param name for setting specified range for {}",
              name);
          MACH3LOG_INFO("Range for {} = [{:.2f}, {:.2f}]", name, lower, upper);
        }
      }

      // Cross-section and flux parameters have boundaries that we scan between,
      // check that these are respected in setting lower and upper variables
      ...
      MACH3LOG_INFO(
          "Scanning {} with {} steps, from [{:.2f} , {:.2f}], prior = {:.2f}",
          name, n_points, lower, upper, prior);

      // Make the TH1D
      auto hScan = std::make_unique<TH1D>((name + "_full").c_str(),
                                          (name + "_full").c_str(), n_points,
                                          lower, upper);
      hScan->SetTitle((std::string("2LLH_full, ") + name + ";" + name +
                       "; -2(ln L_{sample} + ln L_{xsec+flux} + ln L_{det})")
                          .c_str());
      hScan->SetDirectory(nullptr);

      auto hScanSam = std::make_unique<TH1D>((name + "_sam").c_str(),
                                             (name + "_sam").c_str(), n_points,
                                             lower, upper);
      hScanSam->SetTitle((std::string("2LLH_sam, ") + name + ";" + name +
                          "; -2(ln L_{sample})")
                             .c_str());
      ...
      for (unsigned int ivs = 0; ivs < samples.size(); ++ivs) {
        std::string NameTemp = samples[ivs]->GetName();
        hScanSample[ivs] = std::make_unique<TH1D>(
            (name + "_" + NameTemp).c_str(), (name + "_" + NameTemp).c_str(),
            n_points, lower, upper);
        hScanSample[ivs]->SetDirectory(nullptr);
        hScanSample[ivs]->SetTitle(("2LLH_" + NameTemp + ", " + name + ";" +
                                    name + "; -2(ln L_{" + NameTemp + "})")
                                       .c_str());
        nSamLLH[ivs] = 0.;
      }
      ...
      for (unsigned int ivc = 0; ivc < systematics.size(); ++ivc) {
        ...
        hScanCov[ivc] = std::make_unique<TH1D>((name + "_" + NameTemp).c_str(),
                                               (name + "_" + NameTemp).c_str(),
                                               n_points, lower, upper);
        hScanCov[ivc]->SetDirectory(nullptr);
        hScanCov[ivc]->SetTitle(("2LLH_" + NameTemp + ", " + name + ";" + name +
                                 "; -2(ln L_{" + NameTemp + "})")
                                    .c_str());
        nCovLLH[ivc] = 0.;
      }
      ...
      if (PlotLLHScanBySample) {
        ...
        for (unsigned int ivs = 0; ivs < samples.size(); ++ivs) {
          ...
          for (int is = 0; is < samples[ivs]->GetNsamples(); ++is) {
            hScanSamSplit[SampleIterator] =
                new TH1D((name + samples[ivs]->GetSampleTitle(is)).c_str(),
                         (name + samples[ivs]->GetSampleTitle(is)).c_str(),
                         n_points, lower, upper);
            hScanSamSplit[SampleIterator]->SetTitle((std::string("2LLH_sam, ") +
                                                     name + ";" + name +
                                                     "; -2(ln L_{sample})")
                                                        .c_str());
            SampleIterator++;
      ...
      for (int j = 0; j < n_points; ++j) {
        ...
        if (PlotLLHScanBySample) {
          ...
          for (unsigned int ivs = 0; ivs < samples.size(); ++ivs) {
            for (int is = 0; is < samples[ivs]->GetNsamples(); ++is) {
              hScanSamSplit[SampleIterator]->SetBinContent(
                  j + 1, 2 * sampleSplitllh[SampleIterator]);
              SampleIterator++;
            }
          }
        }
...
void FitterBase::GetStepScaleBasedOnLLHScan() {
  ...
  if (Sample_LLH->IsZombie()) {
    MACH3LOG_WARN("Couldn't find Sample_LLH, it looks like LLH scan wasn't "
                  "run, will do this now");
    RunLLHScan();
  }

  for (ParameterHandlerBase *cov : systematics) {
    const int npars = cov->GetNumSystematicParams();
    ...
    for (int i = 0; i < npars; ++i) {
      ...
      const double LLH_val =
          std::max(LLHScan->GetBinContent(1),
                   LLHScan->GetBinContent(LLHScan->GetNbinsX()));
      // If there is no sensitivity leave it
      if (LLH_val < 0.001)
        continue;

      // EM: assuming that the likelihood is gaussian, approximate sigma value
      // is given by variation/sqrt(-2LLH) can evaluate this at any point,
      // simple to evaluate it in the first bin of the LLH scan KS: We assume
      // variation is 1 sigma, each dial has different scale so it becomes
      // faff...
      const double Var = 1.;
      const double approxSigma = TMath::Abs(Var) / std::sqrt(LLH_val);

      // Based on Ewan comment I just took the 1sigma width from the LLH,
      // assuming it was Gaussian, but then had to also scale
      // by 2.38/sqrt(N_params)
...
void FitterBase::Run2DLLHScan() {
  ...
  // Number of points we do for each LLH scan
  const int n_points = GetFromManager<int>(
      fitMan->raw()["LLHScan"]["2DLLHScanPoints"], 20, __FILE__, __LINE__);
  // We print 5 reweights
  const int countwidth = int(double(n_points) / double(5));
  ...
  const double nSigma = GetFromManager<int>(
      fitMan->raw()["LLHScan"]["LLHScanSigma"], 1., __FILE__, __LINE__);
  ...
  for (ParameterHandlerBase *cov : systematics) {
    ...
    for (int i = 0; i < npars; ++i) {
      ...
      if (isScanRanges) {
        ...
        if (it != scanRanges.end() &&
          ...
          MACH3LOG_INFO(
              "Found matching param name for setting specified range for {}",
              name_x);
          MACH3LOG_INFO("Range for {} = [{:.2f}, {:.2f}]", name_x, lower_x,
                        upper_x);
        }
      }
      ...
      // KS: Check if we want to skip this parameter
      if (CheckSkipParameter(SkipVector, name_x))
        continue;

      for (int j = 0; j < i; ++j) {
        std::string name_y = cov->GetParFancyName(j);
        // KS: Check if we want to skip this parameter
        if (CheckSkipParameter(SkipVector, name_y))
          continue;
        ...
        // If param ranges are specified in scanRanges node, extract it from
        // there
        if (isScanRanges) {
          // Find matching entries through std::maps
          ...
          if (it != scanRanges.end() &&
            ...
            MACH3LOG_INFO(
                "Found matching param name for setting specified range for {}",
                name_y);
            MACH3LOG_INFO("Range for {} = [{:.2f}, {:.2f}]", name_y, lower_y,
                          upper_y);
          }
        }

        // Cross-section and flux parameters have boundaries that we scan
        // between, check that these are respected in setting lower and upper
        // variables
        lower_y = std::max(lower_y, cov->GetLowerBound(j));
        upper_y = std::min(upper_y, cov->GetUpperBound(j));
        MACH3LOG_INFO(
            "Scanning X {} with {} steps, from {:.2f} - {:.2f}, prior = {}",
            name_x, n_points, lower_x, upper_x, prior_x);
        MACH3LOG_INFO(
            "Scanning Y {} with {} steps, from {:.2f} - {:.2f}, prior = {}",
            name_y, n_points, lower_y, upper_y, prior_y);

        auto hScanSam = std::make_unique<TH2D>(
            (name_x + "_" + name_y + "_sam").c_str(),
            (name_x + "_" + name_y + "_sam").c_str(), n_points, lower_x,
            upper_x, n_points, lower_y, upper_y);
        hScanSam->SetDirectory(nullptr);
...
void FitterBase::RunSigmaVar() {
  ...
  // Checking each mode is time consuming so we only consider one which are
  // relevant for particular analysis
  constexpr int nRelevantModes = 2;

  bool DoByMode = GetFromManager<int>(fitMan->raw()["General"]["DoByMode"],
                                      false, __FILE__, __LINE__);

  // KS: If true it will make additional plots with LLH sample contribution in
  // each bin, should make it via config file...
  ...
  for (ParameterHandlerBase *cov : systematics) {
    ...
    for (int i = 0; i < cov->GetNumSystematicParams(); ++i) {
      ...
      for (int j = 0; j < numVar; ++j) {
        ...
        double paramVal =
            cov->GetParInit(i) + sigmaArray[j] * std::sqrt((*Cov)(i, i));

        // Check the bounds on the parameter
        paramVal = std::max(cov->GetLowerBound(i),
                            std::min(paramVal, cov->GetUpperBound(i)));
        ...
        for (unsigned int ivs = 0; ivs < samples.size(); ivs++) {
          for (int k = 0; k < samples[ivs]->GetNsamples(); ++k) {
            ...
            auto currSamp = M3::Clone<TH2Poly>(
                static_cast<TH2Poly *>(samples[ivs]->GetPDF(k)));
            // Set a descriptiv-ish title
            std::string title_long =
                std::string(currSamp->GetName()) + "_" + parVarTitle;

            // Enable the mode histograms AFTER addSelection is called
            // Get the 1d binning we want. Let's just use SetupBinning to get
            // this as it already exists
            std::vector<double> xbins;
            std::vector<double> ybins;
            samples[ivs]->SetupBinning(M3::int_t(k), xbins, ybins);

            // KS:here we loop over all reaction modes defined in
            // "RelevantModes[nRelevantModes]"
            if (DoByMode) {
              // KS: this is only relevant if PlotByMode is turned on
              // Checking each mode is time consuming so we only consider one
              // which are relevant for particular analysis
              MaCh3Modes_t RelevantModes[nRelevantModes] = {
                  samples[ivs]->GetMaCh3Modes()->GetMode("CCQE"),
                  samples[ivs]->GetMaCh3Modes()->GetMode("2p2h")};

              sigmaArray_mode_x[j][SampleIterator] =
                  new TH1D *[nRelevantModes]();
              sigmaArray_mode_y[j][SampleIterator] =
                  new TH1D *[nRelevantModes]();
              // Now get the TH2D mode variations
              std::string mode_title_long;

              for (int ir = 0; ir < nRelevantModes; ir++) {
                auto currSampMode = M3::Clone<TH2Poly>(static_cast<TH2Poly *>(
                    samples[ivs]->GetPDFMode(k, RelevantModes[ir])));

                mode_title_long =
                    title_long + "_" +
                    samples[ivs]->GetMaCh3Modes()->GetMaCh3ModeName(
                        RelevantModes[ir]);
                currSampMode->SetNameTitle(mode_title_long.c_str(),
                                           mode_title_long.c_str());
                dirArrySample[SampleIterator]->cd();
                currSampMode->Write();

                sigmaArray_mode_x[j][SampleIterator][ir] = PolyProjectionX(
                    currSampMode.get(), (mode_title_long + "_xProj").c_str(),
                    xbins);
                sigmaArray_mode_x[j][SampleIterator][ir]->SetDirectory(nullptr);
                sigmaArray_mode_y[j][SampleIterator][ir] = PolyProjectionY(
                    currSampMode.get(), (mode_title_long + "_yProj").c_str(),
                    ybins);
                sigmaArray_mode_y[j][SampleIterator][ir]->SetDirectory(nullptr);
              }
            }

            // KS: This will give different results depending if data or Asimov,
            // both have their uses
            if (PlotLLHperBin) {
              auto currLLHSamp = M3::Clone<TH2Poly>(
                  static_cast<TH2Poly *>(samples[ivs]->GetPDF(k)));
              currLLHSamp->Reset("");
              ...
              TH2Poly *Datapdf =
                  static_cast<TH2Poly *>(samples[ivs]->GetData(k));
              TH2Poly *W2pdf = samples[ivs]->GetW2(k);

              for (int bin = 1; bin < currLLHSamp->GetNumberOfBins() + 1;
                   bin++) {
                const double mc = MCpdf->GetBinContent(bin);
                const double dat = Datapdf->GetBinContent(bin);
                const double w2 = W2pdf->GetBinContent(bin);
                currLLHSamp->SetBinContent(
                    bin, samples[ivs]->GetTestStatLLH(dat, mc, w2));
              }
              currLLHSamp->SetNameTitle((title_long + "_LLH").c_str(),
                                        (title_long + "_LLH").c_str());
              dirArrySample[SampleIterator]->cd();
              currLLHSamp->Write();
            }

            // Project down onto x axis
            sigmaArray_x[j][SampleIterator] =
                std::unique_ptr<TH1D>(PolyProjectionX(
                    currSamp.get(), (title_long + "_xProj").c_str(), xbins));
            sigmaArray_x[j][SampleIterator]->SetDirectory(nullptr);
            sigmaArray_x[j][SampleIterator]->GetXaxis()->SetTitle(
                currSamp->GetXaxis()->GetTitle());
            sigmaArray_y[j][SampleIterator] =
                std::unique_ptr<TH1D>(PolyProjectionY(
                    currSamp.get(), (title_long + "_yProj").c_str(), ybins));
            sigmaArray_y[j][SampleIterator]->SetDirectory(nullptr);
            sigmaArray_y[j][SampleIterator]->GetXaxis()->SetTitle(
                currSamp->GetYaxis()->GetTitle());

            sigmaArray_x_norm[j][SampleIterator] =
                M3::Clone<TH1D>(sigmaArray_x[j][SampleIterator].get());
            sigmaArray_x_norm[j][SampleIterator]->Scale(1., "width");
            sigmaArray_y_norm[j][SampleIterator] =
                M3::Clone<TH1D>(sigmaArray_y[j][SampleIterator].get());
      ...
      for (unsigned int ivs = 0; ivs < samples.size(); ivs++) {
        for (int k = 0; k < samples[ivs]->GetNsamples(); ++k) {
          std::string title =
              std::string(samples[ivs]->GetPDF(k)->GetName()) + "_" + name;
          auto var_x = MakeAsymGraph(sigmaArray_x[1][SampleIterator].get(),
                                     sigmaArray_x[2][SampleIterator].get(),
                                     sigmaArray_x[3][SampleIterator].get(),
                                     (title + "_X").c_str());
          auto var_y = MakeAsymGraph(sigmaArray_y[1][SampleIterator].get(),
                                     sigmaArray_y[2][SampleIterator].get(),
                                     sigmaArray_y[3][SampleIterator].get(),
                                     (title + "_Y").c_str());

          auto var_x_norm =
              MakeAsymGraph(sigmaArray_x_norm[1][SampleIterator].get(),
                            sigmaArray_x_norm[2][SampleIterator].get(),
                            sigmaArray_x_norm[3][SampleIterator].get(),
                            (title + "_X_norm").c_str());
          auto var_y_norm =
              MakeAsymGraph(sigmaArray_y_norm[1][SampleIterator].get(),
                            sigmaArray_y_norm[2][SampleIterator].get(),
                            sigmaArray_y_norm[3][SampleIterator].get(),
                            (title + "_Y_norm").c_str());
          ...
          if (DoByMode) {
            ...
            for (int ir = 0; ir < nRelevantModes; ir++) {
              auto var_mode_x = MakeAsymGraph(
                  sigmaArray_mode_x[1][SampleIterator][ir],
                  sigmaArray_mode_x[2][SampleIterator][ir],
                  sigmaArray_mode_x[3][SampleIterator][ir],
                  (title + "_" +
                   samples[ivs]->GetMaCh3Modes()->GetMaCh3ModeName(
                       RelevantModes[ir]) +
                   "_X")
                      .c_str());
              auto var_mode_y = MakeAsymGraph(
                  sigmaArray_mode_y[1][SampleIterator][ir],
                  sigmaArray_mode_y[2][SampleIterator][ir],
                  sigmaArray_mode_y[3][SampleIterator][ir],
                  (title + "_" +
                   samples[ivs]->GetMaCh3Modes()->GetMaCh3ModeName(
                       RelevantModes[ir]) +
                   "_Y")
...
// *************************
// For comparison with P-Theta we usually have to apply different parameter
// values then usual 1, 3 sigma
void FitterBase::CustomRange(const std::string &ParName, const double sigma,
                             double &ParamShiftValue) const {
  // *************************
  if (!fitMan->raw()["SigmaVar"]["CustomRange"])
    return;
  ...
  if (Config[ParName] && Config[ParName][sigmaStr]) {
    ...
    MACH3LOG_INFO("  ::: setting custom range from config ::: {} -> {}",
                  ParName, ParamShiftValue);
  }
}
...
void WriteHistogramsByMode(SampleHandlerFD *sample, const std::string &suffix,
                           const bool by_mode, const bool by_channel,
    ...
    for (int iDim = 0; iDim < sample->GetNDim(iSample); iDim++) {
      ...
      if (by_mode) {
        for (int iMode = 0; iMode < modes->GetNModes(); ++iMode) {
          auto modeHist = sample->Get1DVarHistByModeAndChannel(
              iSample, ProjectionName, iMode);
          WriteHistograms(modeHist, sampleName + "_" +
                                        modes->GetMaCh3ModeName(iMode) +
                                        ProjectionSuffix + suffix);
          delete modeHist;
        }
      }
      ...
      if (by_channel) {
        for (int iChan = 0; iChan < sample->GetNOscChannels(iSample); ++iChan) {
          auto chanHist = sample->Get1DVarHistByModeAndChannel(
              iSample, ProjectionName, -1,
              iChan); // -1 skips over mode plotting
          WriteHistograms(chanHist, sampleName + "_" +
                                        sample->GetFlavourName(iSample, iChan) +
                                        ProjectionSuffix + suffix);
          delete chanHist;
        }
      }
      ...
      if (by_mode && by_channel) {
        for (int iMode = 0; iMode < modes->GetNModes(); ++iMode) {
          for (int iChan = 0; iChan < sample->GetNOscChannels(iSample);
               ++iChan) {
            auto hist = sample->Get1DVarHistByModeAndChannel(
                iSample, ProjectionName, iMode, iChan);
            WriteHistograms(hist, sampleName + "_" +
                                      modes->GetMaCh3ModeName(iMode) + "_" +
                                      sample->GetFlavourName(iSample, iChan) +
                                      ProjectionSuffix + suffix);
            delete hist;
          }
      ...
      if (!by_mode && !by_channel) {
        ...
        if (iDim == 1) {
          auto hist2D =
              sample->Get2DVarHist(iSample, sample->GetXBinVarName(iSample),
                                   sample->GetYBinVarName(iSample));
...
void FitterBase::RunSigmaVarFD() {
  ...
  bool plot_by_mode =
      GetFromManager<bool>(fitMan->raw()["SigmaVar"]["PlotByMode"], false);
  bool plot_by_channel =
      GetFromManager<bool>(fitMan->raw()["SigmaVar"]["PlotByChannel"], false);
  auto SkipVector = GetFromManager<std::vector<std::string>>(
      fitMan->raw()["SigmaVar"]["SkipVector"], {}, __FILE__, __LINE__);

  if (plot_by_mode)
    MACH3LOG_INFO("Plotting by sample and mode");
  if (plot_by_channel)
    MACH3LOG_INFO("Plotting by sample and channel");
  if (!plot_by_mode && !plot_by_channel)
    MACH3LOG_INFO("Plotting by sample only");
  if (plot_by_mode && plot_by_channel)
    MACH3LOG_INFO("Plotting by sample, mode and channel");

  auto SigmaArray = GetFromManager<std::vector<double>>(
      fitMan->raw()["SigmaVar"]["SigmaArray"], {-3, -1, 0, 1, 3}, __FILE__,
      __LINE__);
  if (std::find(SigmaArray.begin(), SigmaArray.end(), 0.0) ==
      SigmaArray.end()) {
    MACH3LOG_ERROR(":: SigmaArray does not contain 0! Current contents: {} ::",
                   fmt::join(SigmaArray, ", "));
    throw MaCh3Exception(__FILE__, __LINE__);
  }
  ...
  for (size_t s = 0; s < systematics.size(); ++s) {
    for (int i = 0; i < systematics[s]->GetNumSystematicParams(); i++) {
      ...
      // KS: Check if we want to skip this parameter
      if (CheckSkipParameter(SkipVector, ParName))
        continue;
      ...
      for (unsigned int iSample = 0; iSample < samples.size(); ++iSample) {
        ...
        if (!MaCh3Sample) {
          MACH3LOG_ERROR(":: Sample {} do not inherit from  SampleHandlerFD "
                         "this is not implemented::",
                         samples[i]->GetSampleTitle(iSample));
          throw MaCh3Exception(__FILE__, __LINE__);
        }
        ...
        for (int SampleIndex = 0; SampleIndex < MaCh3Sample->GetNsamples();
          ...
          SampleDir[SampleIndex] =
              ParamDir->mkdir(MaCh3Sample->GetSampleTitle(SampleIndex).c_str());
        }
        ...
        for (size_t j = 0; j < SigmaArray.size(); ++j) {
          ...
          double ParamShiftValue =
              ParamNomValue +
              sigma * std::sqrt((*systematics[s]->GetCovMatrix())(i, i));
          ParamShiftValue =
              std::max(std::min(ParamShiftValue, ParamUpper), ParamLower);

          /// Apply custom range to make easier comparison with p-theta
          CustomRange(ParName, sigma, ParamShiftValue);

          MACH3LOG_INFO("  - set to {:<5.2f} ({:<2} sigma shift)",
                        ParamShiftValue, sigma);
          ...
          if (sigma == 0) {
            ...
            suffix =
                "_" + ParName + "_sig_" + sign + sigmaStr + "_val_" + valueStr;
          }

          systematics[s]->SetParProp(i, ParamShiftValue);
          MaCh3Sample->Reweight();

          WriteHistogramsByMode(MaCh3Sample, suffix, plot_by_mode,
                                plot_by_channel, SampleDir);
        }
        for (int subSampleIndex = 0;
             subSampleIndex < MaCh3Sample->GetNsamples(); ++subSampleIndex) {

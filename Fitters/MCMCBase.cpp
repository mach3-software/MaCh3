...
MCMCBase::MCMCBase(manager *man) : FitterBase(man) {
  ...
  out_of_bounds = false;
  chainLength = Get<unsigned>(fitMan->raw()["General"]["MCMC"]["NSteps"],
                              __FILE__, __LINE__);

  AnnealTemp = GetFromManager<double>(
      fitMan->raw()["General"]["MCMC"]["AnnealTemp"], -999);
  if (AnnealTemp < 0)
    anneal = false;
  else {
...
void MCMCBase::RunMCMC() {
  ...
  for (const auto &syst : systematics) {
    if (syst->GetDoAdaption()) {
      throw std::runtime_error("Adaption not yet implemented");
    }
  }
}
...
void MCMCBase::PrintProgress(bool StepsPrint) {
  // *******************
  if (StepsPrint)
    MACH3LOG_INFO("Step:\t{}/{}, current: {:.2f}, proposed: {:.2f}",
                  step - stepStart, chainLength, logLCurr, logLProp);
  if (StepsPrint)
    MACH3LOG_INFO(
        "Accepted/Total steps: {}/{} = {:.2f}", accCount, step - stepStart,
        static_cast<double>(accCount) / static_cast<double>(step - stepStart));
  ...
  if (debug) {
    debugFile << "\n-------------------------------------------------------"
              << std::endl;
    debugFile << "Step:\t" << step + 1 << "/" << chainLength
              << "  |  current: " << logLCurr << " proposed: " << logLProp
              << std::endl;
  }
#endif
}
...
#ifdef DEBUG
  debugFile << " logLProp: " << logLProp << " logLCurr: " << logLCurr
            << " acc_prob: " << acc_prob << " fRandom: " << fRandom
            << std::endl;
#endif

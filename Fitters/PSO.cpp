...
PSO::PSO(manager *man) : LikelihoodFit(man) {
  // ***************
  AlgorithmName = "PSO";
  fConstriction = Get<double>(fitMan->raw()["General"]["PSO"]["Constriction"],
                              __FILE__, __LINE__);
  fInertia = Get<double>(fitMan->raw()["General"]["PSO"]["Inertia"], __FILE__,
                         __LINE__) *
             fConstriction;
  fOne =
      Get<double>(fitMan->raw()["General"]["PSO"]["One"], __FILE__, __LINE__) *
      fConstriction;
  fTwo =
      Get<double>(fitMan->raw()["General"]["PSO"]["Two"], __FILE__, __LINE__) *
      fConstriction;
  fParticles = Get<int>(fitMan->raw()["General"]["PSO"]["Particles"], __FILE__,
                        __LINE__);
  fIterations = Get<int>(fitMan->raw()["General"]["PSO"]["Iterations"],
                         __FILE__, __LINE__);
  fConvergence = Get<double>(fitMan->raw()["General"]["PSO"]["Convergence"],
                             __FILE__, __LINE__);

  fDim = 0;

  if (fTestLikelihood) {
    fDim = Get<int>(fitMan->raw()["General"]["PSO"]["TestLikelihoodDim"],
                    __FILE__, __LINE__);
  }
}
...
void PSO::RunMCMC() {
  ...
  if (fTestLikelihood) {
    ...
    for (int i = 0; i < fDim; ++i) {
      ...
      outTree->Branch(Form("Parameter_%d", i), paramlist[i],
                      Form("Parameter_%d[nParts]/D", i));
    }
...
void PSO::init() {
  ...
  for (int i = 0; i < fDim; i++) {
    MACH3LOG_INFO("Variable {} : {:.2f}, {:.2f}", i, ranges_min[i],
                  ranges_max[i]);
  }
...
PSO::bisection(const std::vector<double> &position, const double minimum,
  ...
  for (unsigned int i = 0; i < position.size(); ++i) {
    ...
    std::vector<double> new_position = position;
    new_position[i] = position[i] - range;
    double val_1 = CalcChi(new_position) - minimum - 1.0;
    while (val_1 * -1.0 > 0.0) {
      new_position[i] -= range;
      val_1 = CalcChi(new_position) - minimum - 1.0;
    }
    std::vector<double> bisect_position = position;
    bisect_position[i] =
        bisect_position[i] - (position[i] - new_position[i]) / 2;
    std::vector<std::vector<double>> position_list{new_position,
                                                   bisect_position, position};
    ...
    while (res > precision) {
      if (value_list[0] * value_list[1] < 0) {
        ...
        new_bisect_position[i] =
            new_bisect_position[i] +
            (position_list[1][i] - position_list[0][i]) / 2;
        ...
        new_bisect_position[i] +=
            (position_list[2][i] - position_list[1][i]) / 2;
        double new_val = CalcChi(new_bisect_position) - minimum - 1.0;
    ...
    std::vector<double> new_position_p = position;
    new_position_p[i] = position[i] + range;
    ...
    std::vector<double> bisect_position_p = position;
    bisect_position_p[i] = bisect_position_p[i] +=
        (new_position_p[i] - position[i]) / 2;
    std::vector<std::vector<double>> position_list_p{
    ...
    while (res_p > precision) {
      if (value_list_p[0] * value_list_p[1] < 0) {
        std::vector<double> new_bisect_position_p = position_list_p[0];
        new_bisect_position_p[i] +=
            (position_list_p[1][i] - position_list_p[0][i]) / 2;
        ...
        std::vector<double> new_bisect_position_p = position_list_p[1];
        new_bisect_position_p[i] +=
            (position_list_p[2][i] - position_list_p[1][i]) / 2;
        double new_val_p = CalcChi(new_bisect_position_p) - minimum - 1.0;
    ...
    uncertainties_list.push_back(
        {std::abs(position[i] - position_list[1][i]),
         std::abs(position[i] - position_list_p[1][i])});
    MACH3LOG_INFO("Uncertainty finished for d = {}", i);
    MACH3LOG_INFO("LLR values for ± positive and negative uncertainties are "
                  "{:<10.2f} and {:<10.2f}",
...
// *************************
std::vector<std::vector<double>>
PSO::calc_uncertainty(const std::vector<double> &position,
...
double PSO::swarmIterate() {
  ...
  for (int i = 0; i < fParticles; ++i) {
    std::vector<double> part1 =
        vector_multiply(system[i]->get_velocity(), fInertia);
    std::vector<double> part2 =
        vector_multiply(vector_subtract(system[i]->get_personal_best_position(),
                                        system[i]->get_position()),
                        (fOne * random->Rndm()));
    std::vector<double> part3 = vector_multiply(
        vector_subtract(get_best_particle()->get_personal_best_position(),
                        system[i]->get_position()),
        (fTwo * random->Rndm()));
    std::vector<double> new_velocity =
        three_vector_addition(part1, part2, part3);
    std::vector<double> new_pos =
        vector_add(system[i]->get_position(), new_velocity);
    transform(total_pos.begin(), total_pos.end(), new_pos.begin(),
              total_pos.begin(), [](double x, double y) { return x + y; });
    ...
    for (int j = 0; j < fDim; ++j) {
      ...
      if (fixed[j])
        new_pos[j] = system[i]->get_position()[j];
    }
  ...
  std::vector<double> best_pos =
      get_best_particle()->get_personal_best_position();
  std::vector<double> result(best_pos.size(), 0.0);
  transform(total_pos.begin(), total_pos.end(), total_pos.begin(),
            [=](double val) { return val / fParticles; });
  transform(total_pos.begin(), total_pos.end(), best_pos.begin(),
            result.begin(), [](double x, double y) { return x - y; });
...
void PSO::run() {
  ...
  for (int i = 0; i < fIterations; ++i, ++iter) {
    ...
    // Weight inertia randomly but scaled by total distance of swarm from global
    // minimum - proxy for total velocity fWeight =
    // ((random->Rndm()+1.0)*0.5)*(10.0/meanVel);
    ...
    if (i % 100 == 0) {
      ...
      for (int j = 0; j < fDim; ++j) {
        MACH3LOG_INFO("    Dim {} = {:<10.2f}", j,
                      get_best_particle()->get_personal_best_position()[j]);
      }
    }
    if (fConvergence > 0.0) {
  ...
  MACH3LOG_INFO("Best LLR: {:.2f}",
                get_best_particle()->get_personal_best_value());
  uncertainties =
      bisection(get_best_particle()->get_personal_best_position(),
                get_best_particle()->get_personal_best_value(), 0.5, 0.005);
  MACH3LOG_INFO("Position for Global Minimum = ");
  for (int i = 0; i < fDim; ++i) {
    MACH3LOG_INFO("    Dim {} = {:<10.2f} +{:.2f}, -{:.2f}", i,
                  get_best_particle()->get_personal_best_position()[i],
                  uncertainties[i][1], uncertainties[i][0]);
  }
}
...
void PSO::WriteOutput() {
  ...
  if (fTestLikelihood) {
    ...
    for (auto &parhandlr : systematics) {
      {
        for (int i = 0; i < parhandlr->GetNumProposalParams();
             ++i, ++ParCounter) {
          double ParVal = minimum[ParCounter];
          // KS: Basically apply mirroring for parameters out of bounds
          (*PSOParValue)(ParCounter) = ParVal;
          (*PSOParError)(ParCounter) =
              (uncertainties[ParCounter][0] + uncertainties[ParCounter][1]) /
              2.0;
          // KS: For fixed params HESS will not calcuate error so we need to
          // pass prior error
          if (parhandlr->IsParameterFixed(i)) {
            (*PSOParError)(ParCounter) = parhandlr->GetDiagonalError(i);
          }
        }
      }
    }
...
double PSO::CalcChi2(const double *x) {
  ...
  if (fTestLikelihood) {
    ...
    return LikelihoodFit::CalcChi2PC(x);
  }
}

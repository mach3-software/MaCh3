...
namespace M3{
    ...
    double ReweightModule::Graph_interpolateNO(TGraph2D* graph, double theta13, double dm32){
        if (!graph) {
    ...
    double ReweightModule::Graph_interpolateIO(TGraph2D* graph, double theta13, double dm32){
        if (!graph) {
            MACH3LOG_ERROR("Graph pointer is null");
            throw MaCh3Exception(__FILE__, __LINE__);
        }
    ...
    double ReweightModule::Graph_interpolate1D(TGraph* graph, double theta13){
    ...
    bool ReweightModule::GetParameterInfo(MCMCProcessor* processor, const std::string& paramName, double& mean, double& sigma, bool& isFlat){
    ...
    void ReweightModule::LoadReweightingSettings(std::vector<ReweightConfig>& reweightConfigs, const YAML::Node& reweight_settings) {
            ...
            if (reweightConfig.dimension == 1) {
                if (ReweightType == "Gaussian") {
                    ...
                    if (paramNames.empty() || allPriorValues.empty() || paramNames.size() != allPriorValues.size()) {
                        MACH3LOG_ERROR("Invalid Gaussian reweight configuration for {}: {} parameters, {} prior pairs", reweightKey, paramNames.size(), allPriorValues.size());
                        continue;
                    }
                }
    ...
    double ReweightModule::Get1DWeight(const ReweightConfig& rwConfig, const std::map<std::string, double>& paramValues) {
        double weight = 1.;
    ...
    double ReweightModule::Get2DWeight(const ReweightConfig& rwConfig, const std::map<std::string, double>& paramValues) {
        double weight = 1.;
    ...
    void ReweightModule::ReweightMCMC(const std::string& configFile, const std::string& inputFile){
        ...
        for (const auto& rwConfig : reweightConfigs) {
            ...
            weightBranches[rwConfig.weightBranchName] = outTree->Branch(rwConfig.weightBranchName.c_str(), &weights[rwConfig.weightBranchName], (rwConfig.weightBranchName + "/D").c_str());
    ...
    ReweightModule::~ReweightModule() = default;
    
    MaCh3ArgumentParser* ReweightModule::get_parser(){
        m_parser = std::make_unique<MaCh3ArgumentParser>("reweight", "1.0", argparse::default_arguments::help);
        m_parser->add_description("Tool for MCMC reweighting.");
